...
async function testTransfer() {
  ...
  console.log('ERC20 转账测试 - LINK 代币 (已优化 Gas 费用)');
  console.log('='.repeat(80));
  console.log('\n转账信息:');
  ...
  console.log('\n优化说明:');
  console.log('- Max Priority Fee: 从 2 gwei 降低到 1.5 gwei');
  console.log('- Max Fee: 从 baseFee * 2 降低到 baseFee * 1.3');
  console.log('- 这样可以降低预估的 Gas 成本');
  console.log('\n' + '='.repeat(80));
  ...
  // 2. 请求用户确认并输入私钥
  console.log('\n' + '='.repeat(80));
  console.log('⚠️  准备执行转账，请确认以上信息无误！');
  console.log('='.repeat(80));

  rl.question('\n请输入发送者私钥 (按 Ctrl+C 取消): ', async (privateKey) => {
    try {
      // 确保私钥格式正确
      ...
      if (account.address.toLowerCase() !== FROM_ADDRESS.toLowerCase()) {
        console.error(`\n❌ 错误: 私钥对应的地址不匹配！`);
        console.error(`预期地址: ${FROM_ADDRESS}`);
        console.error(`实际地址: ${account.address}`);
        console.error('\n这就是导致 "exceeds balance" 错误的原因！');
        rl.close();
        process.exit(1);
      }
      ...
      console.log('='.repeat(80));
      console.log('\n已验证的功能:');
      console.log('✅ 1. 私钥验证和账户创建');
      console.log('✅ 2. 查询 ETH 余额');
      console.log('✅ 3. 查询 ERC20 代币余额');
      console.log('✅ 4. 构建 EIP-1559 交易（优化 Gas）');
